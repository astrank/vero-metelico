...
const Obra: NextPage<ObraProps> = ({ posts, categorias }) => {
    ...
    if (query !== undefined && !Array.isArray(query)) {
      ...
      categorias?.map((c) => {
        if (query == c.nombre_plural || query == c.nombre_singular) {
          ...
          setObras(
            posts.filter(
              (p) => p.categoria.nombre_singular === c.nombre_singular
            )
          );
        }
      });
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between">
      <Head>
        <title>{`${
          categoria ? categoria.nombre_plural : "Obra"
        } | Verónica Metélico`}</title>
        <meta
          name="description"
      ...
      <div className="flex flex-col gap-6 text-primary-900 mx-4 my-8 md:mx-10 lg:mx-14 xl:mx-44 mb-auto">
        <h1 className="font-asap text-3xl">
          {categoria ? categoria.nombre_plural : "Últimas publicaciones"}
        </h1>
        <div className="flex flex-col gap-4 my-6">
          {obras &&
            ...
            obras
              ...
              .map((obra, i) => (
                <Link
                  href={`/obra/${obra.slug.current}`}
                  key={i}
                  className="group flex flex-col gap-3"
                >
                  <h2 className="self-start font-asap text-2xl group-hover:text-primary-700">
                    {obra.titulo}
                  </h2>

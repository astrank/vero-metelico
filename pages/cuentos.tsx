...
const Cuentos: NextPage<CuentosProps> = ({ posts }) => {
  return (
    <div className="min-h-screen flex flex-col justify-between">
      <Head>
        <title>Cuentos | Verónica Metélico</title>
        <meta
          name="description"
            ...
            posts.map((post, i) => (
              <div key={i} className="group flex flex-col gap-3">
                <Link href={`/obra/${post.slug}`} className="self-start">
                  <h2 className="font-asap text-2xl group-hover:text-primary-700">
                    {post.title}
                  </h2>
                </Link>
...
export const getStaticProps = async () => {
  ...
  return {
    props: {
      posts: posts.filter((post) => post.category == "Cuento"),
    },
  };
};
